...
export function WebsitePaymentsClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/waitlist", {
        ...
        body: JSON.stringify({ 
          email, 
          source: "website-payments",
          product: "website_payments" 
        }),
      });
  ...
  if (isSubmitted) {
    return (
      <div className="bg-[#11AD30]/5 border border-[#11AD30]/20 rounded-2xl p-8">
        ...
        <p className="text-neutral-600">We&apos;ll email you as soon as website payments are available.</p>
      </div>
    );
  }
